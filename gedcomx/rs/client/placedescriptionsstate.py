...
class PlaceDescriptionsState(GedcomxApplicationState):

    def __init__(self, session, request, response, access_token, state_factory):
        super(PlaceDescriptionsState, self).__init__(session, request, response, access_token, state_factory)

    def reconstruct(self, request, response):
        return PlaceDescriptionsState(self.session, request, response, self.accessToken, self.stateFactory)
    ...
    def addPlaceDescription(self, placeDescription):
        link = self.getLink(Rel.SELF)

        if link is None or link.href == "":
            return None
        entity = Gedcomx()
        entity.places.append(placeDescription)
