...
class PersonsState(GedcomxApplicationState):

    def __init__(self, session, request, response, access_token, state_factory):
        super(PersonsState, self).__init__(session, request, response, access_token, state_factory)

    def reconstruct(self, request, response):
        return PersonsState(self.session, request, response, self.accessToken, self.stateFactory)
