...
class PersonSpousesState(GedcomxApplicationState):
    """PersonSpousesState"""

    def __init__(self, session, request, response, accessToken, stateFactory):
        super(PersonSpousesState, self).__init__(session, request, response, accessToken, stateFactory)

    def reconstruct(self, request, response):
        return PersonSpousesState(self.session, request, response, self.accessToken, self.stateFactory)
    ...
    def findRelationShipTo(self, spouse):
        """
        :param Person spouse: Object referring to the parent.
        :return:
        """
        relationships = self.getRelationShips()
        """:type : ListProperty(Relationship)"""

        if relationships is None or spouse.id == "":
            return None

        for relationship in relationships:
            if relationship.person2.resource.endswith(spouse.id) or relationship.person1.resource.endswith(spouse.id):
                return relationship
        return None
    ...
    def readSpouse(self, spouse):
        """

        :param Person spouse: Spouse
        :return:
        """
        link = spouse.getLink(Rel.PERSON)
        if link is None:
            link = spouse.getLink(Rel.SELF)
    ...
    def readAncestryWithSpouse(self, person):
        pass

    def readDescendencyWithSpouse(self, person):
        pass